        ...
        <div className={'box p-lg-5 p-2 pt-5 mb-3 w-100 d-flex flex-column align-items-center'}>
            ...
            <div className={'d-flex flex-column flex-lg-row justify-content-around w-100'}>
                <div className={'has-validation'}>
                    ...
                    <input type="text" className={`form-control ${invalid.name ? 'is-invalid' : ''}`}
                           value={state.data.name} onChange={(e) => {
                ...
                <div className={'has-validation'}>
                    ...
                    <input type="text" className={`form-control ${invalid.family ? 'is-invalid' : ''}`}
                           value={state.data.family} onChange={(e) => {
                        setEditKey(editKey + 1)
                ...
                <div className={'has-validation'}>
                    ...
                    <Select value={state.data.gender} options={[{id: 'MALE', name: 'مرد'}, {id: 'FEMALE', name: 'زن'}]}
                            className={`form-control ${invalid.gender ? 'is-invalid' : ''}`} onChange={(v) => {
                        setEditKey(editKey + 1)
            ...
            <div className={'d-flex flex-column flex-lg-row align-items-center'}>
                <div className={'col-lg-6'}>
                    <p className={' alert alert-info my-3'}>مهمترین فاکتور در انتخاب رشته سهمیه‌های آزاد، رتبه در سهمیه
                        در ضریب‌های مختلف است.</p>
                </div>
                <div className={'col-lg-6 w-100 d-flex justify-content-center my-3'}>
                    <div className={'box col-lg-8'}>
                        ...
                        <button type={'button'} className={'btn btn-info'} onClick={() => {
                            setShowModal(true)
                        }}>مشاهده راهنمای محل رتبه ها در کارنامه
                        </button>
                    </div>
                </div>
            </div>
                            ...
                            return <tr>
...

                                {
                                    state.data.share.id === 2 && <td>
                                        <div className={'has-validation'}>
                                            <InputNumber
                                                className={`form-control ${invalid.zaribha2[index] ? 'is-invalid' : ''}`}
                                                type="integer" value={item.rotbeBiSahmie} onchange={(v) => {
                                                setEditKey(editKey + 1)
                                                let temp = state.data.ranks;
            ...
            <div>
                ...
                <button className={'btn btn-info mx-2'} onClick={() => history.replace(getUrl(routes.first))}>مرحله
                    قبل
                </button>
            </div>
        </div>
